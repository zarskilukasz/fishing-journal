...
import { createGroundbaitsService } from "@/lib/services/equipment.service";
import { createErrorResponse, createSuccessResponse, createNoContentResponse } from "@/lib/api/error-response";

/**
 * GET /api/v1/groundbaits/{id}
 *
 * Gets a single groundbait by ID.
 * Requires authentication. Returns 404 if the groundbait doesn't exist or belongs to another user.
 *
 * Path parameters:
 * - id: UUID of the groundbait
 */
...
/**
 * PATCH /api/v1/groundbaits/{id}
 *
 * Updates an existing groundbait (partial update).
 * Requires authentication. Returns 404 if the groundbait doesn't exist or belongs to another user.
 *
 * Path parameters:
 * - id: UUID of the groundbait
...
/**
 * DELETE /api/v1/groundbaits/{id}
 *
 * Soft-deletes a groundbait by setting deleted_at.
 * Requires authentication. Returns 404 if the groundbait doesn't exist or belongs to another user.
 *
 * Path parameters:
 * - id: UUID of the groundbait
